...
from dataloader_cam16 import C16Dataset
from dataloader import XIEHE_Dataset

from modules import attmil, clam, dsmil, transmil, mean_max, clam_with_Transformer
...
def one_fold(args, k, ckc_metric, train_p, train_l, test_p, test_l, val_p, val_l):
    ...
    if args.datasets.lower() == 'tcga':
        ...
        train_set = XIEHE_Dataset(train_p[k], train_l[k], args.tcga_max_patch, args.dataset_root,
                                persistence=args.persistence, keep_same_psize=args.same_psize, is_train=True)
        test_set = XIEHE_Dataset(test_p[k], test_l[k], args.tcga_max_patch, args.dataset_root,
                               persistence=args.persistence, keep_same_psize=args.same_psize)
        if args.val_ratio != 0.:
            val_set = XIEHE_Dataset(val_p[k], val_l[k], args.tcga_max_patch, args.dataset_root,
                                  persistence=args.persistence, keep_same_psize=args.same_psize)
        else:
            val_set = test_set
    ...
    if args.fix_loader_random:
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, sampler=RandomSampler(train_set),
                                  num_workers=args.num_workers)

    val_loader = DataLoader(val_set, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    print(len(train_loader),len(test_loader),len(val_loader),"len(train_loader),len(test_loader),len(val_loader)")
    # bulid networks
    ...
    #    model = clam.CLAM_SB(n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)
       ...
       model = clam_mamba_4_path.CLAM_MB_four_path(n_classes=args.n_classes, dropout=args.dropout, act=args.act).to(device)
        ...
        # cpt_mamba_efft = torch.load(model_path)
        # model.load_state_dict(cpt_mamba_efft['model'], strict=False)

        # model = clam_with_Transformer.CLAM_MB_with_Transformer(n_classes=args.n_classes, dropout=args.dropout,
        #                                                      act=args.act).to(device)

        #model = clam_DyHG(n_classes=args.n_classes, dropout=args.dropout, act=args.act).to(device)

    elif args.model == 'transmil':
        model = transmil.TransMIL(n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)

        # from modules.S4MIL import s4model
        # model = s4model.S4Model(model_dim=512, state_dim=32, input_dim=1024, n_classes=2).to(device)

        # from modules.SRMamba import SRMambaMIL
        # model = SRMambaMIL.MambaMIL(n_classes=2, dropout=0.25, act='relu', n_features=1024, layer=2, rate=10,
        #                             type="SRMamba").cuda()
        # print(model.rate,"model.rate")

        #model = TransMIL_4_path.TransMIL(n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)




    elif args.model == 'dsmil':
        i_classifier = dsmil.FCLayer(in_size=512, out_size=args.n_classes).cuda()
        b_classifier = dsmil.BClassifier(input_size=512, output_class=args.n_classes,
                                         dropout_v=args.dropout, nonlinear=0).cuda()
        model = dsmil.MILNet(i_classifier, b_classifier).to(device)
        args.cls_alpha = 0.5
        args.aux_alpha = 0.5
        state_dict_weights = torch.load('modules/init_ckp/dsmil_init.pth')
        ...
        margin, p1, p2 = -0.1, 0.05, 0.01
        i_classifier = mdmil.FCLayer(in_size=1024, out_size=args.n_classes).to(device)
        model = mdmil.MILNet(i_classifier, args.n_classes, margin, p1, p2, CAMEYLON).cuda()

        # for memory bank
    # for memory bank
    elif args.model == 'GraphMIL':
        model = DyHG(in_dim=1024, emb_dim=512, n_classes=args.n_classes, dropout=0.1, hyper_num=16, device=device).to(device)
    ...
    if args.lr_sche == 'cosine':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epoch,
                                                               0) if not args.lr_supi else torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, args.num_epoch * len(train_loader), 0)
    ...
    for epoch in range(epoch_start, args.num_epoch):
        train_loss, start, end = train_loop(args, model, train_loader, optimizer, device, amp_autocast, criterion,
                                            loss_scaler, scheduler, k, epoch)
        train_time_meter.update(end - start)
        stop, accuracy, auc_value, precision, recall, fscore, test_loss = val_loop(args, model, val_loader, device,
                                                                                   criterion, early_stopping, epoch)

        if args.always_test:

            _te_accuracy, _te_auc_value, _te_precision, _te_recall, _te_fscore, _te_test_loss_log = test(args, model,
                                                                                                         test_loader,
                                                                                                         device,
                                                                                                         criterion)
        ...
        if not args.no_log:
            print(
                '\r Epoch [%d/%d] train loss: %.1E, test loss: %.1E, accuracy: %.3f, auc_value:%.3f, precision: %.3f, recall: %.3f, fscore: %.3f , time: %.3f(%.3f)' %
                (epoch + 1, args.num_epoch, train_loss, test_loss, accuracy, auc_value, precision, recall, fscore,
                 train_time_meter.val, train_time_meter.avg))
    ...
    if not args.no_log:
        print(
            ...
            optimal_ac, opt_auc, opt_pre, opt_re, opt_fs))
    ...
    print(acs, pre, rec, fs, auc, te_auc, te_fs,"acs, pre, rec, fs, auc, te_auc, te_fs")
    if args.always_test:
    ...
    for i, data in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.model in ('clam_sb', 'clam_mb'):
                ...
                keep_num = patch_num
            elif args.model in ('dsmil'):
                cls_loss, train_logits = model(bag)
                patch_num, keep_num = 0., 0.

            elif args.model in ('mdmil'):
                temp, momentum = 0.05, 0.2
                memory_loss = HybridMemory(512, args.n_classes, temp=temp, momentum=momentum)
                classes, prediction_bag_conv, train_logits, feats = model(bag)
                feats = F.normalize(feats, dim=-1)
                contrastive_loss = memory_loss(feats[:, int(label), :], label.cuda(), epoch)
                patch_num, keep_num = 0., 0.
            ...
            if args.loss == 'ce':
                ...
                if args.model in ('dsmil'):
                    cls_loss = criterion(cls_loss.view(batch_size, -1), label)

                elif args.model in ('mdmil'):
                    max_prediction, _ = torch.max(classes, 0)
                    bag_loss = criterion(max_prediction[None,], label) + logit_loss
                    logit_loss = bag_loss + 0.5 * contrastive_loss
                    cls_loss = 0
                    args.cls_alpha = 1
                    args.aux_alpha = 0

            elif args.loss == 'bce':
                logit_loss = criterion(train_logits.view(batch_size, -1),
                                       one_hot(label.view(batch_size, -1).float(), num_classes=2))
                if args.model in ('dsmil'):
                    cls_loss = criterion(cls_loss.view(batch_size, -1),
                                         one_hot(label.view(batch_size, -1).float(), num_classes=2))
        ...
        if i % args.log_iter == 0 or i == len(loader) - 1:
            ...
            if not args.no_log:
                print('[{}/{}] logit_loss:{}, cls_loss:{},  patch_num:{}, keep_num:{} '.format(i, len(loader) - 1,
                                                                                               loss_cls_meter.avg,
                                                                                               loss_cl_meter.avg,
                                                                                               patch_num_meter.avg,
                                                                                               keep_num_meter.avg))
...
def val_loop(args, model, loader, device, criterion, early_stopping, epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            elif args.model in ('clam_mb'):
                # test_logits = model(bag,label,instance_eval=True)
                test_logits, cls_loss, patch_num = model(bag, label, instance_eval=True)
                keep_num = patch_num
                # print("test_logits")
            elif args.model in ('mdmil'):
                classes, prediction_bag_conv, test_logits, feats = model(bag)
                patch_num, keep_num = 0., 0.
            ...
            if args.loss == 'ce':
                if (args.model == 'dsmil' and args.ds_average) or (
                        args.model == 'mhim' and isinstance(test_logits, (list, tuple))):
                    test_loss = criterion(test_logits[0].view(batch_size, -1), label)
                    bag_logit.append(
                        (0.5 * torch.softmax(test_logits[1], dim=-1) + 0.5 * torch.softmax(test_logits[0], dim=-1))[:,
                        1].cpu().squeeze().numpy())
                    ...
                    max_prediction, _ = torch.max(classes, 0)
                    test_loss = criterion(test_logits[0].view(batch_size, -1), label)
                    test_loss = criterion(max_prediction[None,], label) + test_loss
                ...
                if args.model == 'dsmil' and args.ds_average:
                    test_loss = criterion(test_logits.view(batch_size, -1), label)
                    bag_logit.append((0.5 * torch.sigmoid(test_logits[1]) + 0.5 * torch.sigmoid(
                        test_logits[0]).cpu().squeeze().numpy()))
                else:
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if args.model == 'dsmil':
                ...
                # test_logits = model(bag,label,instance_eval=False)
                test_logits, cls_loss, patch_num = model(bag, label, instance_eval=True)
                keep_num = patch_num
            else:
                test_logits = model(bag)
...
if __name__ == '__main__':
    ...
    # Dataset
    parser.add_argument('--datasets', default='tcga', type=str, help='[camelyon16, tcga]')
    parser.add_argument('--dataset_root', default=r'E:\SSFNet\XIEHE\\r50', type=str, help='Dataset root path')
    ...
    parser.add_argument('--max_epoch', default=130, type=int,
                        help='Number of max training epochs in the earlystopping [130]')
    ...
    parser.add_argument('--model', default='transmil', type=str, help='Model name')
