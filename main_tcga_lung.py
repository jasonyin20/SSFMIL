...
import argparse, os
from modules import attmil,clam,dsmil,transmil,mean_max
...
def one_fold(args, k, ckc_metric, train_p, train_l, test_p, test_l, val_p, val_l):
    ...
    if args.datasets.lower() == 'tcga':
        ...
        train_set = TCGADataset(train_p[k], train_l[k], args.tcga_max_patch, args.dataset_root,
                                persistence=args.persistence, keep_same_psize=args.same_psize, is_train=True)
        test_set = TCGADataset(test_p[k], test_l[k], args.tcga_max_patch, args.dataset_root,
                               persistence=args.persistence, keep_same_psize=args.same_psize)
        if args.val_ratio != 0.:
            val_set = TCGADataset(val_p[k], val_l[k], args.tcga_max_patch, args.dataset_root,
                                  persistence=args.persistence, keep_same_psize=args.same_psize)
        else:
            val_set = test_set
    ...
    #    model = clam.CLAM_SB(n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)
        ...
        #model = SRMambaMIL.MambaMIL(n_classes=2, dropout=0.25, act='relu', n_features=512, layer=2, rate=5, type="SRMamba").cuda()

        # model = TransMIL_4_path.TransMIL(n_classes=args.n_classes,dropout=args.dropout,act=args.act).to(device)
        # state_dict_weights = torch.load("/home/wuyi/SSF_MIL_V2/Result/seed=2021/nsclc/Transmil_SSFNET_R50_NSCLC_seed_2025_1/fold_"+str(k)+"_model_best_auc.pt")
        # model.load_state_dict(state_dict_weights['model'], strict=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', default=r'E:\SSFNet\nsclc\\r50', type=str, help='Dataset root path')
