...
class C16Dataset_graph(Dataset):

    def __init__(self, file_name, file_label, root, persistence=False, keep_same_psize=0,
                 is_train=False):
        """
        Args
        :param images:
        ...
        """
        super(C16Dataset_graph, self).__init__()
        ...
        if self.persistence:
            self.feats = [torch.load(os.path.join(root, 'PyG_files', _f + '.pt')) for _f in file_name]
    ...
    def __getitem__(self, idx):
        ...
        if self.persistence:
            ...
            dir_path = os.path.join(self.root, "PyG_files_test")

            file_path = os.path.join(dir_path, self.file_name[idx] + '.pt')
            features = torch.load(file_path)
            #print(features,"12234")
...
class XIEHE_Dataset_Graph(Dataset):
    ...
    def __init__(self, file_name=None, file_label=None,max_patch=-1,root=None,persistence=True,keep_same_psize=0,is_train=False):
        ...
        :param transform: optional transform to be applied on a sample
        """
        super(XIEHE_Dataset_Graph, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.persistence:
            ...
            features = torch.load(os.path.join(self.root,'PyG_files_test',file_path))
        return features , int(label)

class TCGA_Dataset_Graph(Dataset):
    def __init__(self, file_name=None, file_label=None,max_patch=-1,root=None,persistence=True,keep_same_psize=0,is_train=False):
        """
        Args
        :param images:
        :param transform: optional transform to be applied on a sample
        """
        super(TCGA_Dataset_Graph, self).__init__()
...
if __name__ == '__main__':
    ...
    dataset_root = '/media/jaonyin/新加卷/RRT_Data/CAMELYON16 R50'
    ...
    for k in range(fold_start, cv_fold):
        ...
        train_set = C16Dataset_graph(train_p[k],train_l[k], root=dataset_root,persistence=False,
                               keep_same_psize=same_psize, is_train=True)
        ...
        # model = MamMIL2NoGIA(input_dim=1024, n_directions=4, bidirectional=True, embed_dim=512, layer_n=2, d_state=128,
        #                      d_conv=4, expand=2,
        #                      act='relu', dropout=0.,
        #                      num_classes=2).to(device)

        model = MamMIL2GIA(input_dim=1024, n_directions=4, bidirectional=True, embed_dim=512, layer_n=2, d_state=128,
                             d_conv=4, expand=2,
                             act='relu', dropout=0.,
                             num_classes=2).to(device)
