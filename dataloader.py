...
class XIEHE_Dataset(Dataset):
    ...
    def __init__(self, file_name=None, file_label=None,max_patch=-1,root=None,persistence=True,keep_same_psize=0,is_train=False):
        ...
        :param transform: optional transform to be applied on a sample
        """
        super(XIEHE_Dataset, self).__init__()
...
if __name__ == '__main__':
    dataset_root ='/media/wuyi/新加卷/jasonyin/RRT_DATA/XIEHE_R50'
    label_path ='/media/wuyi/新加卷/jasonyin/RRT_DATA/XIEHE_R50/label.csv'
...
#1037
    ...
    for k in range(fold_start, cv_fold):
        ...
        train_set = XIEHE_Dataset(train_p[k], train_l[k], tcga_max_patch, dataset_root,
                                persistence=persistence, keep_same_psize=same_psize, is_train=True)
        test_set = XIEHE_Dataset(test_p[k], test_l[k], tcga_max_patch, dataset_root, persistence=persistence,
                               keep_same_psize=same_psize)
        if val_ratio != 0.:
            val_set = XIEHE_Dataset(val_p[k], val_l[k], tcga_max_patch, dataset_root, persistence=persistence,
                                  keep_same_psize=same_psize)
        else:
            val_set = test_set
        print(len(train_set))
