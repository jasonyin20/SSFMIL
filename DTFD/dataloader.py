...
if __name__ == '__main__':
    dataset_root ='/media/jaonyin/新加卷/TCGA-BRAC/RRT_DATA'
    label_path ='/media/jaonyin/新加卷/RRT_Data/TCGA-BRCA PLIP/label.csv'
...
#1037
    ...
    for k in range(fold_start, cv_fold):
        ...
        train_set = TCGADataset(train_p[k], train_l[k], tcga_max_patch, dataset_root,
                                persistence=persistence, keep_same_psize=same_psize, is_train=True)
        test_set = TCGADataset(test_p[k], test_l[k], tcga_max_patch, dataset_root, persistence=persistence,
                               keep_same_psize=same_psize)
        if val_ratio != 0.:
            val_set = TCGADataset(val_p[k], val_l[k], tcga_max_patch, dataset_root, persistence=persistence,
                                  keep_same_psize=same_psize)
        else:
            val_set = test_set
        print(len(train_set))
