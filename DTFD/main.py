...
from DTFD.dataloader import C16Dataset, TCGADataset
...
def one_fold(params,k,train_p,train_l,test_p,test_l,val_p,val_l,epoch_step):
        ...
        if params.dataset.lower() == 'camelyon16':
            params.tcga_max_patch = -1
            params.persistence = False
            params.same_psize = 0
            train_set = C16Dataset(train_p[k], train_l[k], root=params.dataset_root, persistence=params.persistence,
                                       keep_same_psize=params.same_psize, is_train=True)
            test_set = C16Dataset(test_p[k], test_l[k], root=params.dataset_root, persistence=params.persistence,
                                      keep_same_psize=params.same_psize)
            if params.val_ratio != 0.:
                val_set = C16Dataset(val_p[k], val_l[k], root=params.dataset_root, persistence=params.persistence,
                                         keep_same_psize=params.same_psize)
            else:
                val_set = test_set
            ...
            params.persistence = False
            params.same_psize = 0
            train_set = TCGADataset(train_p[k], train_l[k], params.tcga_max_patch, params.dataset_root,
                                        persistence=params.persistence, keep_same_psize=params.same_psize, is_train=True)
            test_set = TCGADataset(test_p[k], test_l[k], params.tcga_max_patch, params.dataset_root,
                                       persistence=params.persistence, keep_same_psize=params.same_psize)
            if params.val_ratio != 0.:
                val_set = TCGADataset(val_p[k], val_l[k], params.tcga_max_patch, params.dataset_root,
                                          persistence=params.persistence, keep_same_psize=params.same_psize)
            else:
                val_set = test_set
        ...
        if params.fix_loader_random:
            ...
            train_loader = DataLoader(train_set, batch_size=params.batch_size, sampler=RandomSampler(train_set),
                                          num_workers=params.num_workers)
        ...
        print_log(f'training slides: {len(train_loader)}, validation slides: {len(val_loader),},validation slides: {len(test_loader),}', log_file)
        ...
        for ii in range(params.EPOCH):
            ...
            time = train_attention_preFeature_DTFD(classifier=classifier, dimReduction=dimReduction, attention=attention, UClassifier=attCls, train_loader=train_loader, ce_cri=ce_cri,
                                                    optimizer0=optimizer_adam0, optimizer1=optimizer_adam1, epoch=ii, params=params, f_log=log_file, numGroup=params.numGroup, total_instance=params.total_instance, distill=params.distill_type)
            ...
            auc_val,mF1_1,macc_1,mprec_1, mrecal_1,test_loss = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                                            UClassifier=attCls, test_loader=val_loader, criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
            print_log(' ', log_file)
            if params.wandb:
                rowd = OrderedDict([
                    ("val_acc",macc_1),
            ...
            if params.always_test:
                auc_val_test,mF1_1_test,macc_1_test,mprec_1_test, mrecal_1_test,test_loss_test = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                                                UClassifier=attCls, test_loader=test_loader, criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
        ...
        tauc,tf1,tacc,tprec, trecal,_ = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                UClassifier=attCls, test_loader=test_loader, criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
        # auc_val_test, mF1_1_test, macc_1_test, mprec_1_test, mrecal_1_test, test_loss_test = test_attention_DTFD_preFeat_MultipleMean(
        #     classifier=classifier, dimReduction=dimReduction, attention=attention,
        #     UClassifier=attCls, test_loader=test_loader, criterion=ce_cri, epoch=ii, params=params, f_log=log_file,
        #     numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
...
def main(params):
    ...
    for k in range(0, params.cv_fold):
        ...
        best_test_acc,best_test_prec,best_test_recal,best_test_f1,best_test_auc,best_val_auc_test = one_fold(params,k,train_p,train_l,test_p,test_l,val_p,val_l,epoch_step)

        print(
            '\n test accuracy: %.3f ,test auc: %.3f,test precision: %.3f,test recall: %.3f,test fscore: %.3f' % (
                best_test_acc, best_test_auc, best_test_prec, best_test_recal, best_test_f1))
...
def test_attention_DTFD_preFeat_MultipleMean(test_loader, classifier, dimReduction, attention, UClassifier, epoch, criterion=None,  params=None, f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
    ...
    with torch.no_grad():
        ...
        for i, (bag_feats, bag_label) in enumerate(test_loader):
            #print(i)
            tslideLabel = bag_label.to(params.device)
            tfeat = bag_feats.squeeze(0)
            tfeat = tfeat.to(params.device)
...
def train_attention_preFeature_DTFD(train_loader, classifier, dimReduction, attention, UClassifier,  optimizer0, optimizer1, epoch, ce_cri=None, params=None,
                                          f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
    start = time.time()

    SlideNames_list = len(train_loader)
    ...
    for i, (bag_feats,bag_label) in enumerate(train_loader):
        ...
        tfeat_tensor = bag_feats.squeeze(0)
        ...
        if i % params.train_show_freq == 0:
            tstr = 'epoch: {} idx: {}'.format(epoch, i)
...
def reOrganize_mDATA_v2(mDATA,label,dataset,dataset_root):
    ...
    if dataset == 'tcga':
        ...
        for i in range(len(slide_name_list)):
            ...
            _label = 0 if slide_label[i] == 'normal' else 1
            #_label = 0 if slide_label[i] == 'LUAD' else 1
            #_label = 0 if slide_label[i] == 'IDC' else 1
            Label.append(_label)
        ...
        for i,slide_name in enumerate(mDATA):
            ...
            if dataset == 'camelyon16':
                ...
                file_path = os.path.join(dir_path, 'pt_files',slide_name+'.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_root', default='/media/wuyi/新加卷/jasonyin/RRT_DATA/XIEHE_PLIP_512', type=str, help='dataset root path')
    ...
    parser.add_argument('--log_dir', default='/home/wuyi/RRT_MIL/result/XIEHE_PLIP_512/dtfdmil', type=str)  ## log file path
