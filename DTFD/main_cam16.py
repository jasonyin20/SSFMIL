...
def one_fold(params, k, train_p, train_l, test_p, test_l, val_p, val_l, epoch_step, writer):
    ...
    dimReduction = DimReduction(in_chn, params.mDim, numLayer_Res=params.numLayer_Res, dropout=params.dropout,
                                act=params.act).to(params.device)
    ...
    # load data
    SlideNames_train, FeatList_train, Label_train = reOrganize_mDATA_v2(train_p[k], train_l[k], dataset=params.dataset,
                                                                        dataset_root=params.dataset_root)

    if params.val_ratio != 0.:
        SlideNames_val, FeatList_val, Label_val = reOrganize_mDATA_v2(val_p[k], val_l[k], dataset=params.dataset,
                                                                      dataset_root=params.dataset_root)
        if params.always_test:
            SlideNames_test, FeatList_test, Label_test = reOrganize_mDATA_v2(test_p[k], test_l[k],
                                                                             dataset=params.dataset,
                                                                             dataset_root=params.dataset_root)
    else:
        SlideNames_val, FeatList_val, Label_val = reOrganize_mDATA_v2(test_p[k], test_l[k], dataset=params.dataset,
                                                                      dataset_root=params.dataset_root)
    ...
    if params.early_stopping:
        earlystop = EarlyStopping(stop_epoch=70 if params.dataset == 'tcga' else 130,
                                  patience=20 if params.dataset == 'tcga' else 30)
    else:
        earlystop = None
    ...
    for ii in range(params.EPOCH):
        ...
        time = train_attention_preFeature_DTFD(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                               UClassifier=attCls,
        ...
        auc_val, mF1_1, macc_1, mprec_1, mrecal_1, test_loss = test_attention_DTFD_preFeat_MultipleMean(
            classifier=classifier, dimReduction=dimReduction, attention=attention,
            UClassifier=attCls, mDATA_list=(SlideNames_val, FeatList_val, Label_val), criterion=ce_cri, epoch=ii,
    ...
    SlideNames_test, FeatList_test, Label_test = reOrganize_mDATA_v2(test_p[k], test_l[k], dataset=params.dataset,
                                                                     dataset_root=params.dataset_root)
    print_log(f'test slides: {len(SlideNames_test)}', log_file)
    tauc, tf1, tacc, tprec, trecal, _ = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier,
                                                                                 dimReduction=dimReduction,
                                                                                 attention=attention,
                                                                                 UClassifier=attCls, mDATA_list=(
        SlideNames_test, FeatList_test, Label_test), criterion=ce_cri, epoch=ii, params=params, f_log=log_file,
                                                                                 writer=writer,
...
def main(params):
    ...
    if params.cv_fold > 1:
        train_p, train_l, test_p, test_l, val_p, val_l = get_kflod(params.cv_fold, p, l, params.val_ratio,
                                                                   params.val_label_balance)

    acs, pre, rec, fs, auc = [], [], [], [], []
    val_test_auc = []
    for k in range(0, params.cv_fold):

        best_test_acc, best_test_prec, best_test_recal, best_test_f1, best_test_auc, best_val_auc_test = one_fold(
            params, k, train_p, train_l, test_p, test_l, val_p, val_l, epoch_step, writer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_root', default='/home/wuyi/RRT_DATA/CAMELYON16 R50', type=str, help='dataset root path')
    ...
    parser.add_argument('--log_dir', default='/home/wuyi/RRT_MIL/result/cam16/dtfdmil', type=str)  ## log file path
