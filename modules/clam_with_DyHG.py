...
class clam_DyHG(CLAM_SB):
    def __init__(self, gate = True, size_arg = "small", dropout = 0., k_sample=8, n_classes=2,
        ...
        self._fc1 = nn.Sequential(nn.Linear(size[0], size[1]), nn.ReLU(), nn.Dropout(dropout))
        self.hyper_num = 20 #论文默认参数
        self.tau = 0.05
        self.num_layers = 1
        ...
        self.construct_hyper = nn.Sequential(
            nn.Linear(size[1], self.hyper_num),
            nn.ReLU(),
            nn.Dropout(dropout),
