...
class DAttention(nn.Module):
    def __init__(self,n_classes,dropout,act):
        ...
        self.D = 128 #128
        self.K = 1
        self.feature = [nn.Linear(in_features=1024, out_features=512)]
...
class DAttention_v2_mamba(nn.Module):
    def __init__(self, n_classes, dropout, act):
        super(DAttention_v2_mamba, self).__init__()
        self.L = 512  # 512
        self.D = 128  # 128
        self.K = 1


        self.Block_mamba = Block_mamba(dim=1024, drop_path=0.0)
    ...
    def forward(self, x, return_attn=False, no_norm=False):
        ...
        x = self.Block_mamba(x)

        #print(x.shape,"x")
        feature = self.feature(x)
        #print(feature.shape,"x")
...
if __name__ == '__main__':
    ...
    attn_mamba_efft = DAttention_v2_mamba(n_classes=2, dropout=0.25, act='relu').cuda()
    data = torch.randn(1,6000,512).cuda()
    data = attn_mamba_efft(data)
